Simulation reports are saved here. There are a total of 7 reports per simulation, all in the same folder.
Each simulation execution has an unique ID to tell it apart from the others (UNIX timestamp on the moment it ends).

Reports:
- <id>_parameters.csv			parameters used for the simulation
...
- <id>_pedestrians.csv			individual time each pedestrian took to cross a semaphore