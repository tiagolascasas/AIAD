...
public class StateCommunicationBehaviour extends CyclicBehaviour {
	...
	private void processMessages() {
		...
		else if (isCandidate && greenTieBreaker(greenCandidates, allPriorityInformation, new int[] {
				thisAgent.getCurrentEmergencyVehicles(), thisAgent.getCurrentNormalCars(), thisAgent.getID() }))
			thisAgent.switchState(SemaphoreStates.GREEN);
	...
	public boolean findIfGreenCandidate(ArrayList<String> greenCandidates, ArrayList<String[]> allPriorityInformation,
		 ...
		 * System.out.print(Integer.toString(i)); for (int j = 0; j <
		 * INFO_PRIORITY_LENGTH; j++) System.out.print("-" +
		 * allPriorityInformation.get(i)[j]); System.out.println(); }
		 */
